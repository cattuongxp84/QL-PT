...
DB_CONFIG = {
    "server": "ADMIN-PC\DPT",  # Thay bằng tên server của bạn
    "database": "QLMTDPT",
...
class ServerGUI:
    ...
    def create_menu(self):
        ...
        menu_bar.add_command(label="Reload Online Status", command=self.reload_online_status)  # Add reload menu option
    ...
    def create_client_grid(self):
        total_pcs = 65
        icon_size = (80, 60)
        spacing_x, spacing_y = 170, 90  # Adjusted spacing_y to reduce the top margin
        ...
        for pc_number in range(1, total_pcs + 1):
            ...
            self.clients[pc_name] = {"ip": f"192.168.100.{60 + pc_number}", "image_id": img_id, "label": label_id, "x": x, "y": y}
            pc_count += 1
        ...
        if data:
            ...
            if action == "login":
                ...
                try:
                    ...
                    """, (username, password, khoavien))
                    db_conn.commit()
                    client_socket.send(json.dumps({"status": "success", "message": "Registration successful!"}).encode('utf-8'))
                except pyodbc.Error as e:
                    client_socket.send(json.dumps({"status": "error", "message": f"Error during registration: {e}"}).encode('utf-8'))
                ...
                if client_ip in online_machines:
                    ...
                    client_socket.send(json.dumps({"status": "success", "message": "Logout successful!"}).encode('utf-8'))
...
def reload_online_status(gui, db_conn):
    try:
        print("Reloading online status...")  # Debug print
        # Set all clients to offline initially
        for pc_name, data in gui.clients.items():
            gui.update_client_status(data["ip"], "offline")

        cursor = db_conn.cursor()
        cursor.execute("SELECT username, client_ip FROM QLMTDPT.dbo.online_status WHERE status = 'online'")
        records = cursor.fetchall()
        for username, client_ip in records:
            print(f"Checking client {client_ip}...")  # Debug print
            if ping_client(client_ip):  # Verify client status
                print(f"Client {client_ip} is online.")  # Debug print
                online_machines[client_ip] = username
                gui.update_client_status(client_ip, "online")
                update_online_status(username, client_ip, "online", db_conn)  # Ensure DB is updated
            else:
                print(f"Client {client_ip} is offline.")  # Debug print
                # Update the database to set the status to offline if the client is not reachable
                cursor.execute("UPDATE QLMTDPT.dbo.online_status SET status = 'offline' WHERE client_ip = ?", (client_ip,))
                db_conn.commit()
    except pyodbc.Error as e:
        print(f"Error reloading online status: {e}")

def start_server(gui, db_conn):
    reload_online_status(gui, db_conn)  # Reload online status from the database

    server_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    server_socket.bind(('192.168.100.60', 12345))
