...
SERVER_IP = "192.168.200.102"
SERVER_PORT = 12345
CLIENT_IP = "192.168.200.101"  # Bind to all available network interfaces
...
def show_login_form():
    ...
    tk.Label(login_frame, text="Mật khẩu (ddmmyyyy)", font=("Arial", 14), bg="white").pack(pady=5)
    entry_password = tk.Entry(login_frame, show="*", font=("Arial", 12), width=30)
    entry_password.pack(pady=5)

    tk.Button(login_frame, text="Đăng nhập", command=login, font=("Arial", 12), width=20, bg="#4CAF50", fg="white").pack(pady=10)
    tk.Button(login_frame, text="Đăng ký ", command=show_register_form, font=("Arial", 12), width=20, bg="#2196F3", fg="white").pack(pady=10)
...
def show_register_form():
    ...
    tk.Button(login_frame, text="Đăng ký", command=register, font=("Arial", 12), width=20, bg="#4CAF50", fg="white").pack(pady=10)
    tk.Button(login_frame, text="Trở lại Đăng nhập", command=show_login_form, font=("Arial", 12), width=20, bg="#2196F3", fg="white").pack(pady=10)

def open_main_window(username):
    global main_window
    main_window = tk.Tk()
    
    # Get the client machine number and remove the leading '1'
    client_machine_number = CLIENT_IP.split('.')[-1][1:]
    
    # Update the window title to include the client machine number
    main_window.title(f"Máy {client_machine_number}")
    ...
    tk.Label(main_window, text=f"Xin chào, {username}!", font=("Arial", 14)).pack(pady=20)
    tk.Button(main_window, text="Đăng xuất", command=lambda: logout(username), font=("Arial", 12), bg="#f44336", fg="white").pack(pady=10)
